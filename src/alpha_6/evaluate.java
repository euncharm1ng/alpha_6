...
public class evaluate {
	...
	public static void aiTurn(int[][] board, int aiTag, int turn, cor[] logs, int logNum) {
		...
		minmaxFindTopFirstMoves(board, aiTag, turn, depth);
	...
	static int
	minmaxFindTopFirstMoves(int[][] board, int currTag, int turn, int currDepth)
	{
		if(currDepth <= 0) return 0;
		int[][] scoreBoard = new int[19][19];
		cor[] topFirstMoves = new cor[depth], topSecondMoves = new cor[depth];
		int[] topFirstScore = new int[depth], topSecondScore = new int[depth];
		int oppoTag = opponentTag(currTag), bestScore = 0, currScore = 0, bestFirst = 0, bestSecond = 0;
		for(int i = 0; i < depth; i++){ 
			topFirstMoves[i] = new cor(-1, -1); 
			topSecondMoves[i] = new cor(-1, -1);
		}
		...
		for(int i = 0; i < depth; i++){
			...
			for(int j = 0; j < depth; j++){
				currScore += topSecondScore[j];
				board[topSecondMoves[j].getX()][topSecondMoves[j].getY()] = currTag;
				currScore += minmaxFindTopFirstMoves(board, oppoTag, turn+1, currDepth-1);
		...
		if(currDepth == depth) System.out.println("hererherhehr " + topFirstMoves[bestFirst] + " " + topSecondMoves[bestSecond]);
		
		return (currDepth % 2 != 0)? bestScore : 0-bestScore;
	}
			...
			for(int j = 0; j < 19; j++){
				for(int k = 0; k < depth; k++){
					if(scoreBoard[i][j] > topScore[k]){
						int replacex = i, replacey = j, replacescore = scoreBoard[i][j];
						for(; k < depth; k++){
							int tempx = topMoves[k].getX();
							int tempy = topMoves[k].getY();
							int tempscore = topScore[k];
							topMoves[k].x = replacex;
							topMoves[k].y = replacey;
							topScore[k] = replacescore;
							replacex = tempx;
							replacey = tempy;
